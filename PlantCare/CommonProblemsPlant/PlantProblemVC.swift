...
class PlantProblemVC: UIViewController {
...
extension PlantProblemVC :  UITableViewDelegate , UITableViewDataSource {
...
//    }
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if let viewController = vc {
      viewController.nameProplen = Problem
...
//    vc?.name = problems[indexPath.row].name
//    print(problems[indexPath.row].name)
//
//    vc?.image1 = problems[indexPath.row].image
//    vc?.solution = problems[indexPath.row].solution
//
//
